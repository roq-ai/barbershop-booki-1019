...
import { deleteWorkingHourById } from 'apiSdk/working-hours';
...
function RestaurantViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<RestaurantInterface>(
    ...
    () =>
      getRestaurantById(id, {
        relations: ['user', 'employee', 'service', 'working_hour'],
      }),
  );
  ...
  const working_hourHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await deleteWorkingHourById(id);
      await mutate();
    } catch (error) {
      setDeleteError(error);
    }
        ...
        {isLoading ? (
          ...
          <>
            ...
            <Box>
              ...
              {hasAccess('employee', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/employees/create?restaurant_id=${data?.id}`}>
              ...
              {hasAccess('service', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/services/create?restaurant_id=${data?.id}`}>
                      <Button colorScheme="blue" mr="4" as="a">
                        Create
                      </Button>
                    </NextLink>
              ...
              {hasAccess('working_hour', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    <Text fontSize="lg" fontWeight="bold">
                      Working Hours
                    </Text>
                    <NextLink passHref href={`/working-hours/create?restaurant_id=${data?.id}`}>
                      <Button colorScheme="blue" mr="4" as="a">
                        Create
                      </Button>
                    </NextLink>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.working_hour?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/working-hours/view/${record.id}`)}
                            key={record.id}
                          >
                            ...
                            <Td>
                              {hasAccess('working_hour', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                                <NextLink
                                  onClick={(e) => e.stopPropagation()}
                                  passHref
                                  href={`/working-hours/edit/${record.id}`}
                                >
                                  <Button mr={2} as="a" variant="outline" colorScheme="blue" leftIcon={<FiEdit2 />}>
                                    Edit
                                  </Button>
                              ...
                              {hasAccess('working_hour', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                                <IconButton
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    working_hourHandleDelete(record.id);
                                  }}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
